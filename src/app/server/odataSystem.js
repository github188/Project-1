...
//明长然  20161220  获取职位
Object.defineProperty(oDataHandle, 'getPosition', {
    value: function(filters,dispatch, callback) {
        token = Store.get("token");
        serviceAddress = Store.get("serviceUrl");
        if(token != '') {
            ...
            //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
            var rquestUri = serviceAddress+"GetPosition?token="+token+filterString;
            //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
            rquestUri = encodeURI(rquestUri);
            $.ajax({
                type: "get",
...
//明长然  20161216  主管人数据获取
Object.defineProperty(oDataHandle, 'getProFileData', {
    value: function (callback) {
        token = Store.get("token");
        serviceAddress = Store.get("serviceUrl");
        if (token != '') {
            var newObj = {};
            newObj.tableName = "Profile";
            newObj.type = "query";
            newObj.token = token;
...
//明长然  20161216  区域数据获取
Object.defineProperty(oDataHandle, 'getSysAreaData', {
    value: function (callback) {
        token = Store.get("token");
        serviceAddress = Store.get("serviceUrl");
        if (token != '') {
            ...
            newObj.type = "query";
            newObj.token = token;
            newObj.neFilter = {key:"ParentGroupId",value:""};
            newObj.orderby = { key: "CreatedDateTime", sort: "desc" };
            oDataHandle.handle(newObj, function (resp) {
                callback(resp);
            });
...
//明长然  20161219  单位数据
Object.defineProperty(oDataHandle, 'getSysUnitData', {
    value: function (callback) {
        token = Store.get("token");
        serviceAddress = Store.get("serviceUrl");
        if (token != '') {
...
//明长然  20161219  部门数据
Object.defineProperty(oDataHandle, 'getSysDepartmentData', {
    value: function (callback) {
        token = Store.get("token");
        serviceAddress = Store.get("serviceUrl");
        if (token != '') {
            var newObj = {};
            newObj.tableName = "Department";
            newObj.type = "query";
            newObj.token = token;
            newObj.orderby = { key: "CreatedDateTime", sort: "desc" };
            oDataHandle.handle(newObj, function (resp) {
...
//曹志强		20161214	获取组织机构数据接口
Object.defineProperty(oDataHandle, 'getOrganizationData',{
  value: function(callback) {
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
...
//曹志强 	20161208 	删除组织机构数据接口   
Object.defineProperty(oDataHandle, 'deleteOrganizationData',{
  value: function(data, callback) {
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
        //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
        var rquestUri = serviceAddress+"DeleteOrganization?token="+token+"&ORGANIZATIONNAME='"+data.ORGANIZATIONCODE+"'&ORGANIZATIONCODE='"+data.ORGANIZATIONCODE+"'";
        //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
        rquestUri = encodeURI(rquestUri);
        $.ajax({
            type: "get",
...
//曹志强		20161214	获取平台所有角色接口
Object.defineProperty(oDataHandle, 'getOrgRoleData',{
  value: function(callback) {
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
        //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
        var rquestUri = serviceAddress+"GetRole?token="+token;
        //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
        rquestUri = encodeURI(rquestUri);
        $.ajax({
            type: "get",
...
//曹志强 	20161208 	增加组织机构数据接口   
Object.defineProperty(oDataHandle, 'addOrganizationData',{
  value: function(data, callback) {
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
        //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
        var rquestUri = serviceAddress+"SetOrganization?token="+token+"&ORGANIZATIONNAME='"+data.ORGANIZATIONNAME+"'&ORGANIZATIONCODE='"+data.ORGANIZATIONNAME+"'&ORGANIZATIONDES='"+data.ORGANIZATIONDES+"'&ROLES='"+data.ROLES+"'";
        //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
        rquestUri = encodeURI(rquestUri);
        $.ajax({
            type: "get",
...
//曹志强 	20161208 	编辑组织机构数据接口   
Object.defineProperty(oDataHandle, 'editOrganizationData',{
  value: function(data, callback) {
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
        //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
        var rquestUri = serviceAddress+"SetOrganization?token="+token+"&ORGANIZATIONNAME='"+data.ORGANIZATIONNAME+"'&ORGANIZATIONCODE='"+data.ORGANIZATIONNAME+"'&ORGANIZATIONDES='"+data.ORGANIZATIONDES+"'&ROLES='"+data.ROLES+"'&SAFETYGROUPID='"+data.SAFETYGROUPID+"'";
        //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
        rquestUri = encodeURI(rquestUri);
        $.ajax({
            type: "get",
...
//刘家顺 20161214获取角色管理接口
Object.defineProperty(oDataHandle, 'roleManagerData',{
  value: function(callback) {
      token = Store.get("token");
      serviceAddress = Store.get("serviceUrl");       
       if(token != '') {
...
//刘家顺 20161214删除角色管理
Object.defineProperty(oDataHandle, 'deleteRoleData',{
  value: function(deleteRoleName,callback) {
  
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
        //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
        var rquestUri = serviceAddress+"SysRole?OPERATOR_TYPE='DELETE'&ROLE_NAME='"+deleteRoleName+"'&token="+token;
        //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
...
//刘家顺 20161214编辑角色管理
Object.defineProperty(oDataHandle, 'editRoleData',{
  value: function(param,callback) {
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
        //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
        var rquestUri = serviceAddress+"SysRole?OPERATOR_TYPE='EDIT'&ROLE_NAME='"+param.ROLE_NAME+"'&OLD_ROLE_NAME='"+param.OLD_ROLE_NAME+"'&ROLE_DESC='"+param.ROLE_DESC+"'&token="+token;
        //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
         console.log(rquestUri)
        rquestUri = encodeURI(rquestUri);
        $.ajax({
...
//刘家顺 20161214 增加角色管理
Object.defineProperty(oDataHandle, 'adddRoleData',{
  value: function(param,callback) {
    token = Store.get("token");
    serviceAddress = Store.get("serviceUrl");
    if(token != '') {
        //url 地址中可能包括中文字符所以需要对此生成 地址进行转码
        var rquestUri = serviceAddress+"SysRole?OPERATOR_TYPE='ADD'&ORANIZATION_NAME='"+param.ORANIZATION_NAME+"'&ROLE_NAME='"+param.ROLE_NAME+"'&ROLE_DESC='"+param.ROLE_DESC+"'&token="+token;
        //Js的Url中传递中文参数乱码问题，重点：encodeURI编码，decodeURI解码
          console.log(rquestUri)
        rquestUri = encodeURI(rquestUri);
        $.ajax({
