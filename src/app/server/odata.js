...
//曹志强 20161031  注册功能，获取token信息
Object.defineProperty(oDataHandle, 'getUserToken', {
	value: function value() {
		console.log("==========获取token信息============");
		var resultToken = "";
		var urll = "http://" + window.location.host;
		var rquestUri = urll + "/rest/auth/login";
		$.ajax({
			type: "get",
			async: false,
			url: rquestUri,
			...
			error: function error(result) {
				console.log("resultTokenError==" + result);
				resultToken = result;
			}
		});
		return resultToken;
...
//曹志强 20161031  注册功能，获取用户信息
Object.defineProperty(oDataHandle, 'getUserInfo', {
	value: function value(token) {
		console.log("==========开始执行查询接口============");
		var resultStr = "";
		var serviceAddress = Store.get("serviceUrl");
		var rquestUri = serviceAddress + "SetUser?token=" + token + "&OPERATOR_TYPE='GET'";
...
//曹志强 20161107  匿名获取用户名和电话  用户判断用户和电话是否唯一
Object.defineProperty(oDataHandle, 'getLoginUser', {
	value: function value() {
		console.log("==========开始执行查询接口============");
		var resultStr = "";
		var serviceAddress = Store.get("serviceUrl");
		var rquestUri = serviceAddress + "GetLoginUser?OPERATOR_TYPE='GET'&anonymouslogin";
		console.log(rquestUri);
		$.ajax({
			type: "get",
			async: false,
			...
			cache: false,
			success: function success(result) {
				console.log("匿名获取用户名和电话接口返回正确结果==" + result);
				resultStr = result;
			},
			error: function error(result) {
				console.log("匿名获取用户名和电话接口返回错误结果==" + result);
				resultStr = result;
			}
		});
		return resultStr;
...
//曹志强 20161128 调用获取单位和角色信息的信息
Object.defineProperty(oDataHandle, 'getLoginUnit', {
	value: function value(param) {
		console.log("==========开始执行查询接口============");
		var resultStr = "";
		var serviceAddress = Store.get("serviceUrl");
		var rquestUri = serviceAddress + "GetLoginUnit?&FDNAME='"+param+"'&OPERATOR_TYPE='GET'&anonymouslogin";
		console.log(rquestUri);
		$.ajax({
			type: "get",
			async: false,
...
//曹志强 20161112  注册功能，保存用户信息
Object.defineProperty(oDataHandle, 'saveRegisterInfo', {
	value: function value(user) {
		console.log("==========开始执行用户注册保存信息接口============");
		var dataResult = "";
		var token = Store.get("token");
		var serviceAddress = Store.get("serviceUrl");
		var rquestUri = serviceAddress + "AddUser?LOGIN_ID='"+user.username+"'&ORANIZATION_NAME='"+user.employer+"'&LOGIN_PWD='"+user.password+"'&USER_NAME='"+user.name+"'&ROLE_NAME='"
						+user.title+"'&EMAIL='"+user.email+"'&PHONE='"+user.phone+"'&OPERATOR_TYPE='ADD'&anonymouslogin";
			rquestUri = encodeURI(rquestUri);
		$.ajax({
			type: "get",
			async: false,
...
//曹志强 20161114 调用获取功能列表接口
Object.defineProperty(oDataHandle, 'getResourcePermissions', {
	value: function value(user) {
		console.log("==========开始执行用户注册保存信息接口============");
		var dataResult = "";
		var token = Store.get("token");
