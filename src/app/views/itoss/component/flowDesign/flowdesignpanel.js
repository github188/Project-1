...
var FlowDesignPanel = React.createClass({
  ...
  updateImagelabel: function() {
    ...
    try {
    	// [g#管理员, g#工程师, g#甲方, g#关闭, parentNode: g], mdname = "design1482378871886"
  ...
  updatEdges: function() {
    try {
      ...
      function showmenu() {
        ...
        //d3.mouse()将返回鼠标位置相对于 这个容器元素的x、y坐标
        var position = d3.mouse(bodyNode);
        var xx = position[0];
  ...
  //  console.log("submenu");
    ...
    if (that.subvisible == 0) {
      ...
      if(showtip==1 && that.Flowv.length<10){
        mainmenu
          .style('position', 'absolute')
          .style('left', 0 + "px")//新建业务对象位置
          .style('top', 0 + "px")//新建业务对象位置
          .style('display', 'inline-block');
      }
    } else {
      that.subvisible = 0;
  ...
  componentWillMount:function(){
    ...
    var resultdata1=this.props.Resultdata;
      ...
      for (var i = 0; i < resultdata1.length; i++) {
        ...
        if(rr.flag=='1')//{"des":"服务台给工程师分权限","endcx":0,"endcy":0,"flag":"1","from":"管理员","id":"","midPointx":0,"midPointy":0,"nextstatus":"待处理","startcx":0,"startcy":0,"state":"待分配","templateId":"undefined","to":"工程师"}
          this.isjswh=true;//已经存在维护角色
